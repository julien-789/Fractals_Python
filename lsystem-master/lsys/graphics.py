...
class LSystemVisualiser(object):
    def __init__(self, canvas, angle, lstring, steplen):
        ...
        self._table = {
            ...
            LToken.AROUND: lambda: self._cursor.turn_left(180),
    ...
    def getnumber(self,lstring):
        #self._stack2 = []
        #self._stack2.append(self._cursor.save_state())
        #self._cursor.draw(1)
        #self._cursor.turn_right(self._angle)
        ...
        while x==0:            
            ...
            #self._cursor.turn_right(self._angle) 
            #self._cursor.turn_right(self._angle) 
            #self._cursor._set_angle(self._angle)
            #self._cursor.move(1)
            if lstring[token]==")":
                x = 1
        ...
        while x==0:            
            ...
            if lstring[token]==")":
                x = 1
        #self._cursor._add_angle(int(numberstr))        
        if y==y:        
            self._cursor.turn_right(120-float(numberstr))
            #self._cursor.turn_right(120-float(numberstr))
            #self._cursor.turn_right(180-float(numberstr))
            pass
        #if y==2:
        #    self._cursor.turn_right(float(numberstr))
        #if y==1:
        #    self._cursor.turn_right(-1*float(numberstr))
        #self._cursor._set_angle(float(numberstr))
        #self._cursor.move(1)
        #self._cursor.restore_state(self._stack2.pop())        
        #self._cursor.draw(1)
        #print (numberstr)
